...
%}
%%
function[h,L,MX,MED,bw,F,U]=violin_dots(Y,dots,fc,varargin)

%defaults:
%_____________________
xL=[];
if ~exist('dots','var');dots=1;end
if ~exist('fc','var');fc=[1 0.5 0];end
lc='k';
alp=0.5;
mc='k';
medc='r';
...
if dots
    ...
    for k=1:length(Y)
        ...
        f=F(:,k);
        u=U(:,k);
        % find point on violin closest to t
        u2=repmat(u,[1 size(t,2)]);
        d=sqrt((u2-t).^2);
        ii=zeros(size(t));
        for kd=1:length(t)
            try
                ii(kd)=find(d(:,kd)==min(d(:,kd)));
            catch
                disp('meh')
            end
        end
